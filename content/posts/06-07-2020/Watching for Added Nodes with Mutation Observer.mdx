...
---
Earlier this week, I came across a slightly niche use case for MutationObserver that seemed like it might be worth sharing. First a quick refresher on the MutationObserver API. Let's look at a simple case of detecting an attribute change on a target node.

```javascript
/* Step one is to create a MutationObserver instance */
const observer = new MutationObserver(
  mutationList => {
/* The observer takes a callback that will be called on each new batch of mutations.*/
  ...
  mutationList => {
      if (mutationRecord.addedNodes){
        ...
        const nodeImLookingFor =  Array.from(
          addedNodes
        ).flatMap(findAmongDescendants)[0];
        // ... do stuff
      }
    }
  }
